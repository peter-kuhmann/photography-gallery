...
import {type DbUser, DbUserSchema} from "~/infrastructure/database/schemas/User";
...
export async function createUser(userData: CreateInputData<DbUser>): Promise<DbUser> {
    const newUser : DbUser = DbUserSchema.parse({
        ...generateCreateData(),
        ...userData
    })

    const fieldsUpdated = await getDb().hsetnx(REDIS_KEYS.users.emailToIdHash, newUser.email, newUser.id)
...
export async function updateUser(user: DbUser, updatedUserData: UpdateInputData<DbUser>): Promise<DbUser> {
...
export async function getUserByEmail(email: string): Promise<DbUser | null> {
    ...
    if ( !user ) {
        throw new Error("Email was present in email to id hash, but user could not be found.")
    }
