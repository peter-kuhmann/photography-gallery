...
export async function dbSetJson<TYPE extends object>(key: string, value: TYPE, mode: SetMode) {
...
export async function dbGetJson<TYPE extends object>(key: string): Promise<TYPE | null> {
    const rawDocument = await getDb().call("JSON.GET", key, "$") as string | null
