import {createUser, getUserByEmail, updateUser} from "~/infrastructure/database/dao/User";
...
async function main() {
    ...
    if ( seedImagesDir ) {
        const fileNames = (await readdir(import.meta.dir + "/" + seedImagesDir))
            .filter(file => file.endsWith(".jpg") || file.endsWith(".jepg") || file.endsWith(".png"))
            .sort();

        for ( const fileName of fileNames ) {
            console.log(`Uploading image: ${fileName}`);
            const inputFile = Bun.file(import.meta.dir + "/" + seedImagesDir + "/" + fileName)

            const s3ObjectKey = S3_DIRECTORIES.albumPictures.prefix + generateS3ObjectId()
            ...
            const {data: optimizedImage, info: optimizedImageInfo} = await sharpImage.resize(resizeOptions).webp({quality: 70}).toBuffer({resolveWithObject: true})

            const s3ObjectKeyOptimizedImage = s3ObjectKey + "_optimized"
            const s3FileOptimizedImage = getS3Client().file(s3ObjectKeyOptimizedImage)
